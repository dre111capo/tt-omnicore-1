...
module tt_um_omnicore (
    ...
    // Unified Sequential Logic: Drives all registers to avoid multiple driver conflicts
    integer k;
    always @(posedge wb_clk_i or posedge wb_rst_i) begin
        if (wb_rst_i) begin
            addr_page   <= 8'd0;
            ...
            // Self-clearing reset PC signal
            if (reset_pc) begin
                reset_pc <= 1'b0;
                pc       <= 5'd0;
            end

            // 1. CPU Execution State Machine (highest priority for internal register access)
            case (state)
                STATE_IDLE: begin
                    // Check Wishbone command to start CPU
                    ...
                    end else if (wbs_cyc_i && wbs_stb_i && wbs_we_i && !wbs_ack_o && (wbs_adr_i == 5'd0) && (addr_page == 8'd10)) begin
                ...
                STATE_EXECUTE_OP: begin
                    case (opcode)
                        // NAND and NOR operations require 2 execution cycles for timing
                        4'h2, 4'h3: begin
            ...
            // 2. Wishbone Write Transactions (active when CPU is IDLE or not accessing instructions/data registers)
                ...
                if (wbs_adr_i == 5'h1F) begin
                    ...
                    case (addr_page)
                        8'd0: begin // Registers 0 to 3
                            if ((state == STATE_IDLE) && (wbs_adr_i[4:2] < 3'd4)) begin
                        ...
                        8'd2, 8'd3, 8'd4, 8'd5, 8'd6, 8'd7, 8'd8, 8'd9: begin // Instructions 0 to 31
                            if ((state == STATE_IDLE) && (wbs_adr_i[4:2] < 3'd4)) begin
